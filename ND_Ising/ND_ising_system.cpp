...
ND_System::ND_System(int N, int DIM)
    : N(N), DIM(DIM), LEN(static_cast<long long>(pow_int(N, DIM))), 
      state(LEN), gen(rd()), dist(0, LEN - 1), prob(0.0, 1.0) {
    this->init();
}

long long ND_System::pow_int(int base, int exp) {
    long long result = 1;
    for (int i = 0; i < exp; i++) {
        result *= base;
    }
